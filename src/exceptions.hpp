...
public:
    IllegalCharError(const char* fn, const char* detail, int ln) : IError("Illegal Character: ", fn, detail, ln) {
    }
};
...
public:
    explicit SemanticError(const char* fn, const char* detail, int ln) : IError("Error: ", fn, detail, ln) {
    }
};
